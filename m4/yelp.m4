...
if test x"$ITSTOOL" = x; then
  AC_MSG_ERROR([itstool not found; it is required to build documentation for yelp.])
fi

AC_ARG_VAR([XMLLINT],[Path to the `xmllint` command])
AC_CHECK_PROG([XMLLINT],[xmllint],[xmllint])
if test x"$XMLLINT" = x; then
  AC_MSG_ERROR([xmllint not found; it is required to build documentation for yelp.])
fi
